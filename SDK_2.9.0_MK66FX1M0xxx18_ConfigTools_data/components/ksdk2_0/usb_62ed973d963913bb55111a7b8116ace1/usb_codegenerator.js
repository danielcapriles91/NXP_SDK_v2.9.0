...
function DescriptorCreator(componentInstance) {
	...
	function usbInterfaceHandler(interfaceSp) {
		...
		function prepareData(classStr) {
			...
			return {deviceClass: deviceClass, classNr: classNr, idSuffix: idSuffix, interfacesCount: interfacesCount, subClass: subClass, protocol: protocol};
		}
		...
		function processHidReportDescriptor(settingSp) {
			var itemEnum = {kCollection: 0xA1, kDesignator_Index: 0x39, kDesignator_Maximum: 0x59, kDesignator_Minimum: 0x49, kEnd_Collection: 0xC0, kFeature: 0xB1,
				kInput: 0x81, kLogical_Maximum: 0x25, kLogical_Minimum: 0x15, kOutput: 0x91, kPhysical_Maximum: 0x45, kPhysical_Minimum: 0x35,
				kPop: 0xB5, kPush: 0xA5, kReport_Count: 0x95, kReport_Id: 0x85, kReport_Size: 0x75, kSet_Delimiter: 0xA9, kString_Index: 0x79,
				kString_Maximum: 0x99, kString_Minimum: 0x89, kUnit: 0x65, kUnit_Exponent: 0x55, kUsage: 0x09, kUsage_Maximum: 0x29,
				kUsage_Minimum: 0x19, kUsage_Page: 0x05};
			var data = [];
			var size = 0;
			var count = 0;
			...
			items.forEach(function (item) {
				...
				data.push(itemEnum[type]);

				// value is not used in case of End Collection
				if (type !== "kEnd_Collection") {
					data.push(value);
				}
			});

			if (isByteAligned(inputReportLengthBits)) {
				scriptApi.logWarning("HID input report of interface " + interface.index + " is not alligned to bytes");
			}
			if (isByteAligned(outputReportLengthBits)) {
				scriptApi.logWarning("HID output report of interface " + interface.index + " is not alligned to bytes");
			}
			if (isByteAligned(featureReportLengthBits)) {
				scriptApi.logWarning("HID feature report of interface " + interface.index + " is not alligned to bytes");
			}

			// divide reportLength by 8 to get the size in bytes
			return {data: data, inputReportLength: inputReportLengthBits / 8,outputReportLength: outputReportLengthBits / 8,
				featureReportLength: featureReportLengthBits / 8};
		}
		...
		var interface = {
			...
			interfaceCount: classSpecific.interfacesCount,
	...
	switch (modeId) {
		case "host": {
			...
			options.topEndpointNumber = hostSp.valueOf("highest_endpoint_number");
		...
		case "device":
			...
			options.interfaces.forEach(function (interface) {
				...
				switch (interface.implementation) {
					case "kImplementationNone":
						// This case is used to mark implementation file as User file
						if (interface.interfaceClass === CONST.USB_CLASS_CIC) {
							new OutputFile(callbackFolder + fileNameH, coreId).closeNoWrite();
						}
						new OutputFile(callbackFolder + fileNameC, coreId).closeNoWrite();
						break;
